...
Текущее состояние исходников: задачи сохраняются в localStorage браузера и остаются после обновления страницы. Реализованы списки, фильтры, подзадачи, массовые действия, отмена/повтор, «Сводка», календарь день/неделя/месяц с повторяющимися событиями, матрица Эйзенхауэра, Focus Lite и привычки с отметками, сериями и регулярностью. Backend, настоящая авторизация и AI пока не подключены. Данные конкретного браузера в архив исходников не входят.

Для продолжения разработки сначала прочитайте `docs/AI_HANDOFF.md`, затем документ нужного модуля: `docs/PLANNER_TASKS.md`, `docs/PLANNER_CALENDAR.md`, `docs/PLANNER_EISENHOWER.md`, `docs/PLANNER_FOCUS.md` или `docs/PLANNER_HABITS.md`.
...
- [Привычки](docs/PLANNER_HABITS.md)
