...
Текущее состояние исходников: задачи сохраняются в localStorage браузера и остаются после обновления страницы. Реализованы списки, фильтры, подзадачи, массовые действия, отмена/повтор, «Сводка» и календарь день/неделя/месяц. Backend, настоящая авторизация и AI пока не подключены. Данные конкретного браузера в архив исходников не входят.

Для продолжения разработки сначала прочитайте `docs/AI_HANDOFF.md`, затем `docs/PLANNER_TASKS.md` или `docs/PLANNER_CALENDAR.md`. Архив содержит текущие локальные изменения, включая ещё не отправленные в GitHub.
